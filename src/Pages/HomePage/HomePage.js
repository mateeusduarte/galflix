    ...
    if (states.searchedMovie === "") {
      return (
        <EmptySearchContainer>
          ...
          <a onClick={functions.onClickSearchGal}>
            Aqui uma oferta irrecusável
          </a>
        </EmptySearchContainer>
      );
    } else if (states.listOfMovies) {
      return states.listOfMovies.map((movie) => {
        if (movie.Poster.length > 10) {
          return (
            <CardMoviesHome
              ...
              title={movie.Title}
              year={movie.Year}
              onClick={() => goToMoviePage(history, movie.imdbID)}
              id={movie.imdbID}
            />
          );
        }
    ...
    <HomeContainer>
      <InputContainer>
        ...
        <input
          ...
          onChange={functions.onChangeSetSearchedMovie}
          value={states.searchedMovie}
        />
      </InputContainer>
      <FlexMovies>{renderSearchedMovies()}</FlexMovies>
    </HomeContainer>
  );
}
